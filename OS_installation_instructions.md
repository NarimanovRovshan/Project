...
Обязательно создаем резервную копию в Time Machine или в стороннем сервисе, чтобы иметь доступ к точке восстановления или нужным файлам, которые не были перенесены в безопасное место.

Выключаем Mac и запускаем его с зажатыми клавишами Command + Option (Alt) + R для загрузки из раздела восстановления.
...
После запуска следует выбрать Дисковую утилиту.

Здесь можно либо полностью отформатировать системный диск, либо удалить раздел с уже установленной системой, оставив другие разделы с данными.

Возвращаемся в главное меню раздела восстановления и выбираем пункт Переустановить macOS.

Далее, следуя подсказкам мастера установки, производим чистую инсталляцию macOS на отформатированный накопитель.
...
Подключаем флешку или накопитель к Mac и запускаем Дисковую утилиту.

Форматируем флешку в формате Mac OS Extended, предварительно сохранив все нужные данные с нее.

В магазине приложений Mac App Store находим установщик macOS Catalina и скачиваем его на компьютер.

После окончания загрузки установщика запускаем приложение Terminal.

Вводим в командной строке:

sudo

В Finder открываем папку Программы, находим Установка MacOS Catalina и в контекстном меню нажимаем Показать содержимое пакета.

В папке Contents/Resources/ находим файл createinstallmedia и перетаскиваем его в открытое окно Terminal (в команду добавится путь к файлу)

Продолжаем писать команду, добавив следующее:

--volume

Теперь перетаскиваем иконку отформатированной флешки с рабочего стола в окно Terminal (в команду снова добавится путь)

Продолжаем писать команду и добавляем туда следующее:
...
Нажимаем клавишу ввод, подтверждаем действие паролем администратора и вводом “y” после соответствующего запроса.
